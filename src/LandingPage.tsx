      ...
      <main className="flex-1 flex flex-col items-center justify-center p-4 md:p-8">
        <div className="max-w-4xl w-full text-center mt-8 md:mt-0 mb-12 md:mb-16">
          ...
          <p className="text-base md:text-xl text-slate-500 max-w-3xl mx-auto leading-relaxed">
            Raamita liiketoimintaasi hyvin toimivan liiketoiminnansuunnittelutyökalun avulla. Tukenasi on relevantilla datalla sparrattu AI LLM-malli.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 w-full max-w-5xl">
          {/* LTS Card */}
          <motion.div whileHover={{ y: -5 }} className="bg-white p-8 rounded-[32px] shadow-xl border border-black/5 flex flex-col">
            <h2 className="text-2xl font-bold mb-4">LTS</h2>
            <p className="text-slate-500 mb-6 flex-1">Liiketoimintasuunnitelman rakennusalusta.</p>
            <button 
              onClick={() => onLogin(PortalType.LTS)}
              className="w-full bg-black text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-slate-800 transition-all"
            >
          ...
          {/* STRATEGIA Card */}
          <motion.div whileHover={{ y: -5 }} className="bg-white p-8 rounded-[32px] shadow-xl border border-black/5 flex flex-col">
            <h2 className="text-2xl font-bold mb-4">STRATEGIA</h2>
            <p className="text-slate-500 mb-6 flex-1">Strategian tekemisen alusta.</p>
            <button 
