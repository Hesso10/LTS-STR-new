...
import { PortalType, UserRole, UserAccount, SystemKnowledge
} from './types';
import { MessageSquare, X } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { auth, db, handleFirestoreError, OperationType }
from './firebase';
import { onAuthStateChanged } from 'firebase/auth';
import { doc, getDoc, onSnapshot, collection, addDoc,
setDoc, query, where } from 'firebase/firestore';
 
// Simple Error Boundary
class ErrorBoundary extends React.Component<{ children:
React.ReactNode }, { hasError: boolean, error: Error | null }> {
...
className="max-w-md">
            <h1
className="text-4xl font-black mb-4">Hups! Jotain meni
vikaan.</h1>
            <p
className="text-slate-500 mb-8">Sovellus kohtasi odottamattoman
virheen. Kokeile ladata sivu uudelleen.</p>
            {this.state.error
&& (
              <div
className="bg-red-50 text-red-600 p-4 rounded-xl text-left text-sm mb-8
...
{this.state.error.message}
            ...
            <button
onClick={() => window.location.reload()} className="bg-black text-white
px-8 py-4 rounded-2xl font-bold">Lataa sivu uudelleen</button>
          </div>
        </div>
      );
    }
...
const MOCK_USERS: UserAccount[] = [
  { uid: 'admin1', email:
'johannes.hesso@innostapersonaltrainer.fi', displayName: 'Johannes', role:
UserRole.ADMIN },
  { uid: 'teacher1', email: 'opettaja@esimerkki.fi',
displayName: 'Opettaja Matti', role: UserRole.TEACHER },
  { uid: 'student1', email: 'matti@esimerkki.fi',
displayName: 'Matti Meikäläinen', role: UserRole.STUDENT, portalType:
PortalType.STRATEGY, companyName: 'Staffyrules Oy' },
  { uid: 'student2', email: 'sanna@esimerkki.fi',
displayName: 'Sanna Suomalainen', role: UserRole.STUDENT, portalType:
PortalType.LTS, companyName: 'Staffivoimaa Oy' },
];
...
useState(window.innerWidth > 768);
  ...
  const [viewingWorkspaceAs, setViewingWorkspaceAs] =
...
useState<UserAccount[]>(MOCK_USERS);
  const [invites, setInvites] =
useState<any[]>([]);
  const [receivedInvites, setReceivedInvites] =
useState<any[]>([]);
  const [systemKnowledge, setSystemKnowledge] =
useState<SystemKnowledge>(DEFAULT_KNOWLEDGE);
...
console.error("Please check your Firebase configuration.");
  ...
  useEffect(() => {
    let unsubscribeUserDoc: (() => void) | null
= null;
...
UserRole.STUDENT;
            if
(firebaseUser.email === 'johannes@hessonpaja.com') {
              role =
UserRole.ADMIN;
            } else if
...
userDoc.data().role as UserRole;
            ...
            if
(userDoc.metadata.hasPendingWrites && role === UserRole.ADMIN
&& firebaseUser.email !== 'johannes@hessonpaja.com') {
...
= portalType;
            if
(userDoc.exists() && userDoc.data().portalType) {
...
canInviteTeamMembers: userDoc.exists() ? userDoc.data().canInviteTeamMembers :
false,
              invitedBy:
userDoc.exists() ? userDoc.data().invitedBy : undefined
            };
...
'User',
              role:
firebaseUser.email === 'johannes@hessonpaja.com' ? UserRole.ADMIN :
UserRole.STUDENT,
              portalType:
...
null;
        ...
        if (view !== 'LANDING'
&& view !== 'PAYMENT' && !isDemo && !portalType) {
          setView('LANDING');
        }
      }
      setIsAuthReady(true);
...
null;
    let unsubscribeInvites: (() => void) | null
= null;
    let unsubscribeReceivedInvites: (() =>
void) | null = null;
...
console.error('Error fetching users:', error);
        ...
        const invitesQuery = user.role
=== UserRole.ADMIN 
          ? collection(db,
'invites')
          : query(collection(db,
'invites'), where('invitedBy', '==', user.uid));
...
onSnapshot(receivedInvitesQuery, (snapshot) => {
          const
fetchedReceivedInvites: any[] = [];
          snapshot.forEach((doc)
=> {
            const data =
doc.data();
...
unsubscribeReceivedInvites();
  ...
  useEffect(() => {
    const savedPortal =
localStorage.getItem('app_portal');
    const savedKnowledge =
localStorage.getItem('app_knowledge');
    if (savedPortal) setPortalType(savedPortal as
PortalType);
    if (savedKnowledge)
setSystemKnowledge(JSON.parse(savedKnowledge));
  }, []);
...
'settings', 'systemKnowledge'), (docSnap) => {
      if (docSnap.exists()) {
...
portal: PortalType) => {
  ...
  const handleUpdateKnowledge = async (knowledge:
SystemKnowledge) => {
    setSystemKnowledge(knowledge);
    localStorage.setItem('app_knowledge',
...
OperationType.WRITE, 'settings/systemKnowledge');
      }
    }
  };
 
  const sendInviteEmail = async (email: string, name:
string, role: string, inviterName: string) => {
    try {
...
appUrl.replace('ais-dev-', 'ais-pre-');
  ...
  const handleInviteUser = async (name: string, email:
string, role: UserRole, portalType?: PortalType, companyName?: string) => {
    try {
      const inviteId =
...
Date().toISOString(),
      ...
      if (portalType) inviteData.portalType =
portalType;
      if (companyName) inviteData.companyName
= companyName;
 
      await setDoc(doc(db, 'invites',
inviteId), inviteData);
      await sendInviteEmail(email, name,
role, user?.displayName || 'Järjestelmänvalvoja');
    } catch (error) {
...
error);
  ...
  const handleInviteTeamMember = async (name: string,
email: string) => {
    if (!user) return;
    try {
      const inviteId =
Math.random().toString(36).substr(2, 9);
      const actualInviterUid = user.role ===
UserRole.TEAM_MEMBER && user.invitedBy ? user.invitedBy : user.uid;
...
Date().toISOString(),
      ...
      try {
        const targetDocRef = doc(db,
'users', actualInviterUid);
        const targetDocSnap = await
getDoc(targetDocRef);
...
= [...(targetData.teamMembers || []), { name, email }];
...
user.uid) {
            const updatedUser
= { ...user, teamMembers: updatedTeamMembers };
...
localStorage.setItem('app_user', JSON.stringify(updatedUser));
      ...
      await sendInviteEmail(email, name,
'team_member', user.displayName || 'Käyttäjä');
    } catch (error) {
  ...
  const handleToggleTeamInvite = async (id: string,
isInvite: boolean, canInvite: boolean) => {
    try {
      const collectionName = isInvite ?
'invites' : 'users';
      await setDoc(doc(db, collectionName,
id), {
  ...
  const handleRemoveTeamMember = async (emailToRemove:
string) => {
    if (!user) return;
    try {
      const actualInviterUid = user.role ===
UserRole.TEAM_MEMBER && user.invitedBy ? user.invitedBy : user.uid;
      const targetDocRef = doc(db, 'users',
actualInviterUid);
      const targetDocSnap = await
getDoc(targetDocRef);
...
targetDocSnap.data();
        const updatedTeamMembers =
(targetData.teamMembers || []).filter((m: { email: string }) => m.email !==
emailToRemove);
...
{
        return <StrategyPortal
onNavigate={setView} user={user} viewingWorkspaceAs={viewingWorkspaceAs} />;
      }
      return <Dashboard
portalType={PortalType.LTS} onNavigate={setView} user={user} />;
    }
 
    if (view === 'ADMIN' && user?.role ===
UserRole.ADMIN) {
      return (
        <AdminPanel 
          users={users} 
...
onLogin={handleLogin} portalType={portalType || undefined} />;
    if (isVerifying) return <VerificationScreen
email={user?.email || ''} onBackToLogin={() => setIsVerifying(false)} />;
...
showSwitchPortal={user?.role === UserRole.ADMIN}
          onSwitchPortal={() =>
setPortalType(portalType === PortalType.LTS ? PortalType.STRATEGY :
PortalType.LTS)}
          user={user}
        />
        <main className="flex-1
overflow-y-auto relative scroll-smooth flex flex-col">
          {viewingWorkspaceAs
&& (
            <div
className="bg-indigo-600 text-white px-4 py-2 flex items-center
justify-between shadow-md z-50 sticky top-0">
              <div
className="flex items-center gap-2">
                
<span className="text-xs font-bold uppercase
tracking-widest">Katselutila:</span>
                
<span className="text-sm font-medium">Tarkastelet toisen
käyttäjän työtilaa ({viewingWorkspaceAs})</span>
...
<button 
          ...
          <div
className="flex-1 px-3 pt-20 pb-6 md:p-12 max-w-5xl mx-auto w-full
min-h-screen">
...
className="hidden lg:block border-l border-black/5 bg-white z-40
...
<div className="absolute top-0 left-0 right-0 h-12 bg-slate-900 flex
items-center justify-center rounded-t-3xl -mt-4">
...
<motion.button 
  ...
  return (
    <ErrorBoundary>
      <LanguageProvider>
        <div
className="font-sans antialiased text-slate-900 selection:bg-indigo-100
